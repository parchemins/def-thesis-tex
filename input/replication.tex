...
%      \only<2->{\item Chaque éditeur possède une copie locale du document;}
      \only<1>{\item Chaque éditeur est connecté à d'autres éditeurs.}
%      \only<2>{\item \textbf{Chaque éditeur est connecté à d'autres éditeurs.}}
      \vspace{0.5cm}
...
%  \only<2>{\item [$\Rightarrow$] taille de messages $\times$ \textbf{nombre de messages}}
  \end{itemize}
